package com.app.nosql2.web.rest;

import com.app.nosql2.domain.C;
import com.app.nosql2.repository.CRepository;
import com.app.nosql2.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.app.nosql2.domain.C}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CResource {
    ...
    private static final String ENTITY_NAME = "appNoSqlC";
    ...
    public ResponseEntity<C> partialUpdateC(@PathVariable(value = "id", required = false) final Long id, @RequestBody C c)
        ...
        Optional<C> result = cRepository
            ...
            .map(existingC -> {
                if (c.getUid() != null) {
                    existingC.setUid(c.getUid());
                }
                if (c.getEntCCCcod() != null) {
                    existingC.setEntCCCcod(c.getEntCCCcod());
                }
                if (c.getEntCCCdsc() != null) {
                    existingC.setEntCCCdsc(c.getEntCCCdsc());
                }
