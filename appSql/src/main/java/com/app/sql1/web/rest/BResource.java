...
import com.app.sql1.repository.BRepository;
import com.app.sql1.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.app.sql1.domain.B}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class BResource {
    ...
    private static final String ENTITY_NAME = "appSqlB";
    ...
    public ResponseEntity<B> partialUpdateB(@PathVariable(value = "id", required = false) final Long id, @RequestBody B b)
        ...
        Optional<B> result = bRepository
            ...
            .map(existingB -> {
                if (b.getUid() != null) {
                    existingB.setUid(b.getUid());
                }
                if (b.getEntAAAuid() != null) {
                    existingB.setEntAAAuid(b.getEntAAAuid());
                }
                if (b.getEntBBBcod() != null) {
                    existingB.setEntBBBcod(b.getEntBBBcod());
                }
                if (b.getEntBBBdsc() != null) {
                    existingB.setEntBBBdsc(b.getEntBBBdsc());
                }
